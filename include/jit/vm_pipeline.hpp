...
/// Three-stage overlap for threaded JIT dispatch:
///   fetch  — resolve handler index (+ optional branch prefetch)
///   execute — run the handler (may speculatively set `frame::pc`)
///   commit — verify branches, chain fall-through, clear speculative state
...
/// Fetch the handler index for @p pc from @p unit.
